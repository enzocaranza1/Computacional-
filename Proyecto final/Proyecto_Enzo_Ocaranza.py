...
n_estados = 6       # Cantidad de estados a calcular
...
for i in range(n_estados):
    ...
    es_sim = np.allclose(C_nm, C_nm.T, atol=1e-5)
    es_ant = np.allclose(C_nm, -C_nm.T, atol=1e-5)
    
    if es_sim:
        tipo_simetria = "Simétrico"
    elif es_ant:
        tipo_simetria = "Antisimétrico"
    else:
        tipo_simetria = "Degenerado (E)"
